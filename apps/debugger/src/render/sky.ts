 ...
 * 0.62 は「天頂の青が濃く残り、地平線ぎわだけ白く抜ける」ところ。
 *
 * ACES は青をかなり脱色するので、見た目の青さは色そのものより
 * **地平線色をどれだけ暗くするか**で決まる（`weather.ts` の horizon）。
 */
const SKY_GAIN = 0.62;
...
export function createDaylight(look: WeatherLook): {
  ...
  /**
   * 影の濃さ。**1 未満にすると影の中だけが持ち上がる。**
   *
   * 晴れた日の日陰は黒くない。太陽が遮られても空の半球はそのまま見えていて、
   * そこからの散乱光（青い回り込み）が差している。とくに**太陽の周り 20〜30 度の
   * 空は他の何倍も明るい**（Mie 散乱の前方散乱。`sky.ts` の暈と同じ現象）ので、
   * 日陰の面でも「太陽の側を向いているほど明るい」という向き依存が残る。
   *
   * three.js の `shadow.intensity` は影の係数を `mix(1, shadow, intensity)` に
   * するので、0.82 なら日陰でも平行光の 18% が残る。これは**日なたには一切
   * 影響しない**ぶんが効きどころで、半球光や環境マップを一律に上げる（＝日なた
   * ごと明るくして影のコントラストを失う）のとは別の道具である。
   *
   * 18% という値は、太陽の直達照度 3.6 に対して 0.65 — 実測の「日陰の水平面
   * 照度は日なたの 1/6〜1/10」に合う。これ以上上げると影が薄い灰色の染みになる。
   */
  sun.shadow.intensity = 0.82;
  // 空からの光。半球光は「上半分＝空、下半分＝地面の照り返し」という置き方なので、
  // **上を向いた面と下を向いた面で別の色が来る**。下を向いた面（上屋の軒裏、
  // 桁の下、車体の裾）はここでしか照らされないので、地面側を落としすぎると
  // 真っ黒になる（実際そうなっていた）。日なたの草地の照り返しは決して暗くない。
